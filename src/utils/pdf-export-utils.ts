...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function buildDoc(sessions: SessionRecord[], range: string): Promise<any> {
    ...
    const WIDTHS  = [60, 40, 90, 90, 55, 48, 44]
...
export async function exportSessionsToPDF(sessions: SessionRecord[], range: string): Promise<void> {
    const doc = await buildDoc(sessions, range)
    ...
    try {
        ...
        return await Promise.race<string | null>([
            ...
            new Promise<null>((_, reject) =>
                setTimeout(() => reject(new Error("PDF preview timed out")), 20000)
            ),
        ])
    } catch (e) {
